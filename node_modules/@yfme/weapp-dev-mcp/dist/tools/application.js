...
import { z } from "zod";
import { buildUrl, connectionContainerSchema, ensureConnectionParameters, formatJson, querySchema, toSerializableValue, toTextResult, } from "./common.js";
...
export function createApplicationTools(manager) {
    return [
...
function createEnsureConnectionTool(manager) {
    return {
        name: "mp_ensureConnection",
        description: "检查小程序自动化会话是否就绪。可选择覆盖连接设置或强制重连。",
        parameters: ensureConnectionParameters,
        execute: async (rawArgs, context) => {
            const args = ensureConnectionParameters.parse(rawArgs ?? {});
            const result = await manager.withMiniProgram(context.log, {
...
function createNavigateTool(manager) {
    return {
        name: "mp_navigate",
        description: "在小程序内导航，支持 navigateTo、redirectTo、reLaunch、switchTab 和 navigateBack。",
        ...
        execute: async (rawArgs, context) => {
            ...
            return manager.withMiniProgram(context.log, { overrides }, async (miniProgram) => {
                let url;
                let page;
                if (transition === "navigateBack") {
                    ...
                    if (!providedPath) {
                        throw new UserError("参数 path 是必需的，除非 transition 是 navigateBack。");
                    }
...
function createScreenshotTool(manager) {
    return {
        name: "mp_screenshot",
        description: "截取当前小程序视口的截图。默认返回内联图片，或保存到文件路径。",
        ...
        execute: async (rawArgs, context) => {
            ...
            return manager.withMiniProgram(context.log, { overrides: args.connection }, async (miniProgram) => {
                const output = await miniProgram.screenshot(args.path ? { path: args.path } : undefined);
...
function createCallWxMethodTool(manager) {
    return {
        name: "mp_callWx",
        ...
        execute: async (rawArgs, context) => {
            ...
            return manager.withMiniProgram(context.log, { overrides: args.connection }, async (miniProgram) => {
                ...
                const result = await miniProgram.callWxMethod(args.method, ...callArgs);
                return toTextResult(formatJson({
                    method: args.method,
                    arguments: callArgs,
...
function createGetConsoleLogsTool(manager) {
    return {
        name: "mp_getLogs",
...
function createCurrentPageTool(manager) {
    return {
        name: "mp_currentPage",
        description: "获取当前页面的信息，包括路径、查询参数、尺寸和滚动位置。withData 为 true 时额外返回页面数据。",
