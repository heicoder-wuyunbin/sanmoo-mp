...
import { z } from "zod";
import { connectionContainerSchema, formatJson, resolveElement, summarizeElement, toSerializableValue, toTextResult, waitOnPage, } from "./common.js";
...
export function createElementTools(manager) {
    return [
...
function createTapElementTool(manager) {
    return {
        name: "element_tap",
        ...
        execute: async (rawArgs, context) => {
            ...
            return manager.withPage(context.log, { overrides: args.connection }, async (page) => {
                const element = await resolveElement(page, args.selector, args.innerSelector);
                await element.tap();
                await waitOnPage(page, waitMs);
                return toTextResult(`已点击元素 "${args.selector}"${args.innerSelector ? ` -> "${args.innerSelector}"` : ""}${waitMs ? ` 并等待 ${waitMs}ms` : ""}。`);
            });
        },
    };
}
function createInputTextTool(manager) {
    return {
        name: "element_input",
        ...
        execute: async (rawArgs, context) => {
            ...
            return manager.withPage(context.log, { overrides: args.connection }, async (page) => {
                ...
                return toTextResult(`已向元素 "${args.selector}"${args.innerSelector ? ` -> "${args.innerSelector}"` : ""} 输入值 "${args.value}"。`);
            });
        },
    };
}
function createCallElementMethodTool(manager) {
    return {
...
function createGetElementDataTool(manager) {
    return {
...
function createSetElementDataTool(manager) {
    return {
        ...
        execute: async (rawArgs, context) => {
            ...
            return manager.withPage(context.log, { overrides: args.connection }, async (page) => {
                ...
                return toTextResult(`已更新组件数据键: ${dataKeys.length ? dataKeys.join(", ") : "(无)"}。`);
            });
        },
    };
}
function createGetInnerElementTool(manager) {
    return {
        ...
        execute: async (rawArgs, context) => {
            ...
            return manager.withPage(context.log, { overrides: args.connection }, async (page) => {
                ...
                if (!innerElement) {
                    throw new UserError(`在元素 "${args.selector}" 内未找到选择器 "${args.targetSelector}" 对应的元素。`);
                }
...
function createGetInnerElementsTool(manager) {
    return {
        ...
        execute: async (rawArgs, context) => {
            ...
            return manager.withPage(context.log, { overrides: args.connection }, async (page) => {
                const element = await resolveElement(page, args.selector, args.innerSelector);
                if (typeof element.$$ !== "function") {
                    throw new UserError(`元素 "${args.selector}" 不支持查询内部元素数组。`);
                }
                const innerElements = await element.$$(args.targetSelector);
                if (!Array.isArray(innerElements)) {
                    throw new UserError(`在元素 "${args.selector}" 内查询选择器 "${args.targetSelector}" 失败。`);
                }
                const elementsInfo = await Promise.all(innerElements.map(async (el, index) => {
...
function createGetElementWxmlTool(manager) {
    return {
        ...
        execute: async (rawArgs, context) => {
            ...
            return manager.withPage(context.log, { overrides: args.connection }, async (page) => {
                ...
                if (typeof element[methodName] !== "function") {
                    throw new UserError(`元素 "${args.selector}" 不支持获取 ${methodName}。`);
                }
...
function createGetElementStylesTool(manager) {
    return {
        ...
        execute: async (rawArgs, context) => {
            ...
            return manager.withPage(context.log, { overrides: args.connection }, async (page) => {
                ...
                if (typeof element.style !== "function") {
                    throw new UserError(`元素 "${args.selector}" 不支持获取样式。`);
                }
                const styles = {};
                await Promise.all(args.names.map(async (name) => {
                    try {
...
function createScrollToTool(manager) {
    return {
        ...
        execute: async (rawArgs, context) => {
            ...
            return manager.withPage(context.log, { overrides: args.connection }, async (page) => {
                ...
                if (typeof element.scrollTo !== "function") {
                    throw new UserError(`元素 "${args.selector}" 不支持滚动操作，仅 scroll-view 组件可使用此功能。`);
                }
                await element.scrollTo(args.x, args.y);
                return toTextResult(`已将元素 "${args.selector}"${args.innerSelector ? ` -> "${args.innerSelector}"` : ""} 滚动到位置 (${args.x}, ${args.y})。`);
            });
        },
    };
}
function createGetAttributesTool(manager) {
    return {
        ...
        execute: async (rawArgs, context) => {
            ...
            return manager.withPage(context.log, { overrides: args.connection }, async (page) => {
                ...
                if (typeof element.attribute !== "function") {
                    throw new UserError(`元素 "${args.selector}" 不支持获取特性。`);
                }
                const attributes = {};
                await Promise.all(args.names.map(async (name) => {
                    try {
...
function createGetBoundingClientRectTool(manager) {
    return {
        ...
        execute: async (rawArgs, context) => {
            ...
            return manager.withMiniProgram(context.log, { overrides: args.connection }, async (miniProgram) => {
                const fullSelector = innerSelector ? `${selector} >>> ${innerSelector}` : selector;
                let result;
                try {
                    result = await miniProgram.evaluate((sel, innerSel) => {
                    ...
                    throw new UserError(`获取元素 "${fullSelector}" 的边界矩形失败: ${message}`);
                }
                return toTextResult(formatJson({
                    selector,
